...
class PesananController extends Controller
{
    ...
    public function update(Request $request, $id)
    {
        $request->validate([
            ...
            'bahan_baku_id.*' => 'nullable|exists:bahan_baku,id',
            'qty.*' => 'nullable|numeric|min:1',
            'remove_bahan_baku_id.*' => 'nullable|exists:detail_pesanan,bahan_baku_id'
        ]);
        ...
        foreach ($bahanBakuIds as $index => $bahanBakuId) {
            ...
            if ($transaksiKeluar) {
                $transaksiKeluar->update(['qty' => $qty]);
            } else {
                TransaksiKeluar::create([
                    'pesanan_id' => $pesanan->id,
                    'bahan_baku_id' => $bahanBakuId,
            ...
            $bahanBaku->update(['qty' => $bahanBaku->qty - $qty]);
        ...
        $removeBahanBakuIds = $request->input('remove_bahan_baku_id', []);
        foreach ($removeBahanBakuIds as $removeBahanBakuId) {
            $detailPesanan = DetailPesanan::where('pesanan_id', $pesanan->id)
                ->where('bahan_baku_id', $removeBahanBakuId)
                ->first();
            if ($detailPesanan) {
